...
import { useState } from "react";
import { ChevronRight, Star, X, Plus, Minus, Check, ShoppingCart } from "lucide-react"; // Added ShoppingCart
import Database from "../Database";
...
const Newproducts = () => {
  ...
  const [cart, setCart] = useState([]); // Client-side cart
  ...
  const handleAddToCart = (product, e) => {
    ...
    if (product.availability === "Out of Stock") {
      showToast(product.message, "success");
    } else {
      setCart((prev) => {
  ...
  const handleAddToCartFromDialog = () => {
    if (selectedProduct.availability === "Out of Stock") {
      showToast(selectedProduct.message, "success");
    } else {
      setCart((prev) => {
  ...
  return (
    <div className="px-[20px] py-[50px] flex flex-col items-center justify-center sm:px-[20px] sm:py-[50px] md:px-[40px] lg:px-[100px] lg:py-[80px]">
      ...
      <div className="flex justify-between flex-wrap">
        {Database.map((card) => {
          console.log("Card icon:", card.icon); // Debug icon field
          return (
            <div
              key={card.id}
              className="mb-[20px] w-full md:w-[48%] lg:w-[30%] xl:w-[23%] relative gap-[4px] h-[400px] lg:mb-8 group"
            >
              <div className="relative w-full h-[300px] overflow-hidden">
                <img
                  ...
                  src={(() => {
                    try {
                      return require("../Images/" + card.img);
                    } catch (e) {
                ...
                <div className="absolute top-0 right-4 h-full flex items-center gap-4 flex-col justify-center transition-all duration-300 group-hover:opacity-100 group-hover:translate-y-0 opacity-0 translate-y-5">
                  <button
                    ...
                    className="p-2 bg-white rounded-md shadow-md hover:scale-110 transition-all hover:bg-blue-500 hover:text-white"
                  >
                    <card.like size={16} />
                  </button>
                  <button
                  ...
                  >
                    {card.icon ? <card.icon size={16} /> : <ShoppingCart size={16} />} {/* Fallback to ShoppingCart */}
                  </button>
                </div>
              </div>

              <p className="text-sm lg:text-[15px] font-montserrat pt-[10px] text-gray-600">{card.name}</p>
              <p className="text-[16px] font-montserratBold pt-[14px] leading-[1.1] lg:text-[18px] group-hover:text-blue-500 transition-colors duration-500">
                {card.type}
              </p>
              ...
              <div className="flex-1 p-6 lg:p-8">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-gray-600">({selectedProduct.rating}/5)</span>
                  </div>
                  <div className="space-y-2">
                    <h5 className="font-montserratBold text-gray-800">Description</h5>
                  ...
                  <div className="flex items-center justify-between pt-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex items-center border rounded-lg border-gray-200">
                        ...
                        <span className="px-4 py-2 border-x border-gray-200 min-w-[50px] text-center text-sm">{quantity}</span>
                        <button
                    ...
                    <div className="text-right">
                      <span className="text-xl lg:text-2xl font-montserratBold text-blue-500">${selectedProduct.price}</span>
                    </div>
                  </div>
                  <div className="flex flex-col sm:flex-row gap-3 pt-6">
                    <button
              ...
              <div className="lg:w-1/2 p-6 lg:p-8">
                <div className="w-full h-[300px] lg:h-[500px] rounded-lg overflow-hidden">
                  <img
                    src={(() => {
                      try {
                        return require("../Images/" + selectedProduct.img);
                      } catch (e) {
