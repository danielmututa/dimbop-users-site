...
import { useAuthStore } from "../../context/userContext" // Original import path
import { toast } from "react-toastify" // Declared the toast variable
...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    if (formData.password !== formData.confirmPassword) {
      toast.error("Passwords do not match!") // Using react-toastify here
      return
    }
    try {
      await authRegister({
        ...
        email: formData.email,
        password: formData.password,
        confirmPassword: formData.confirmPassword, // Corrected key to match useAuthStore
      })
      navigate("/") // Using react-router-dom for navigation
    } catch (error) {
      console.error("Registration error in component:", error)
      // The toast is already handled by useAuthStore, so no need to re-toast here unless specific UI feedback is needed.
    }
  }

  const handleLoginRedirect = () => {
    navigate("/login") // Using react-router-dom for navigation
  }
